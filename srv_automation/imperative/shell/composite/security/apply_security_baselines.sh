...
./srv_automation/atomic/security/patching/automated_patching.sh

echo "--- Firewall setup (skipping) ---"
# ./srv_automation/atomic/security/firewall/configure_firewall.sh

echo "--- SSH hardening ---"
./srv_automation/atomic/security/ssh/ssh_hardening.sh

echo "--- Install and configure Falco (skipping) ---"
# ./srv_automation/composite/security/setup_falco.sh
