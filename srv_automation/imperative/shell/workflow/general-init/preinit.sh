...
./srv_automation/atomic/bootstrap/wait_for_cloud_init.sh

echo "--- Step 2/7: Update and upgrade system packages ---"
./srv_automation/atomic/os/packages/update_and_upgrade.sh
...
#./srv_automation/atomic/secrets/vault/install_vault.sh

echo "--- Step 5/7: Enable cgroups (requires reboot) ---"
./srv_automation/atomic/os/system/enable_cgroups.sh

echo "--- Step 6/7: Install public SSH key(s) ---"
./srv_automation/atomic/security/ssh/install_ssh_key_from_github.sh
...
./srv_automation/atomic/os/reboot/reboot.sh
